...
function readResultForm() {
  ...
  if (!Number.isFinite(obtained) || obtained < 0 || obtained > exam.fullMarks) {
...
function buildRankedResults() {
  const enriched = results
    .map((result) => {
      ...
      const percentage = (Number(result.obtained) / Number(exam.fullMarks)) * 100;
      return {
        ...result,
  ...
  const overallRows = buildOverallRows(enriched.filter((item) => item.published));
...
function renderStudentPortal(roll) {
  ...
  const overall = buildOverallRows(rankedResults.filter((item) => item.published)).find((item) => item.roll === roll);
  ...
  elements.studentHistoryBody.innerHTML = publishedResults.map((item) => `
    <tr>
      ...
      <td data-label="Marks">${item.obtained}/${item.fullMarks} (${item.percentage.toFixed(2)}%)</td>
      <td data-label="GPA">${item.gpa.toFixed(2)} (${item.grade})</td>
      <td data-label="Merit">${ordinal(item.examMerit)}</td>
    </tr>
  `).join("");
}
...
function renderAdminOverview() {
  ...
  const avg = rankedResults.length
    ? rankedResults.reduce((sum, item) => sum + item.percentage, 0) / rankedResults.length
    : 0;
  const top = rankedResults
...
function exportCsv() {
  const header = ["Roll", "Name", "Batch", "Exam", "Date", "Obtained", "Full Marks", "Percentage", "GPA", "Grade", "Merit"];
